...
export default function Contact() {
  ...
  useEffect(() => {
    companyService.get().then((res) => {
      if (res?.data) {
        const d = res.data;
        setContactInfo({
          address: d.address || fallbackContactInfo.address,
          phone: d.phone || fallbackContactInfo.phone,
          email: d.email || fallbackContactInfo.email,
          whatsapp: d.whatsapp || fallbackContactInfo.whatsapp,
        });
        if (d.businessHours?.length) setHours(d.businessHours);
      }
    }).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSending(false);
      const msg = err.response?.data?.error?.message || 'Failed to send message. Please try again or contact us directly via phone/WhatsApp.';
      setError(msg);
    }
  };

  const infoCards = [
    { label: 'Visit Our Showroom', value: contactInfo.address, sub: 'Walk-ins welcome during business hours', icon: contactIcons.address },
    { label: 'Call Us', value: contactInfo.phone, sub: 'Mon \u2013 Sat, 8AM \u2013 6PM', icon: contactIcons.phone },
    { label: 'Email', value: contactInfo.email, sub: 'We respond within 24 hours', icon: contactIcons.email },
    { label: 'WhatsApp', value: contactInfo.whatsapp, sub: 'Quick responses & product inquiries', icon: contactIcons.whatsapp },
  ];
    ...
    <div className="min-h-screen">
      ...
      <section className="relative py-40 pb-20 bg-navy-950 overflow-hidden">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse 50% 60% at 40% 50%, #1e3a5f, transparent)' }}
        />
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
            ...
            <div className="lg:col-span-3">
              <div className="bg-white rounded-2xl shadow-sm border border-cream-200/50 overflow-hidden">
                ...
                <div className="p-8 sm:p-10">
                  <h2 className="font-display text-2xl font-bold text-navy-900 tracking-tight">Send Us a Message</h2>
                  <p className="mt-2 text-sm text-navy-600/60">Fill out the form below and our team will get back to you promptly.</p>
                  ...
                  {submitted ? (
                    <div className="mt-10 text-center py-12 animate-fade-in-up">
                      ...
                      <p className="mt-2 text-sm text-navy-600/60">Thank you for reaching out. We'll respond within 24 hours.</p>
                    </div>
                  ) : (
            ...
            <div className="lg:col-span-2 space-y-6">
              {infoCards.map((info) => (
                <div key={info.label} className="p-6 bg-white rounded-2xl shadow-sm border border-cream-200/50 hover:border-gold-500/20 transition-all duration-300 group">
                ...
                <div className="space-y-3">
                  {hours.map((h) => (
                    <div key={h.day} className="flex items-center justify-between">
                      ...
                      <span className={`text-sm font-medium ${h.time === 'Closed' ? 'text-red-400/80' : 'text-gold-400'}`}>{h.time}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-5 pt-4 border-t border-white/10">
                  <p className="text-xs text-white/40">Holiday hours may vary. Please call ahead for confirmation.</p>
                </div>
              </div>

              {/* Map placeholder */}
              <div className="aspect-[4/3] rounded-2xl overflow-hidden shadow-sm relative" style={{ background: 'linear-gradient(135deg, #1e3a5f, #0f2035)' }}>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-white/15 text-xs mt-1">{contactInfo.address}</p>
                  </div>
                </div>
