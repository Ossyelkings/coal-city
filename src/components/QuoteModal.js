...
export default function QuoteModal({ isOpen, onClose, preselectedProduct }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await quoteService.submit(formData);
      setSubmitted(true);
      setSending(false);
      setTimeout(() => {
        setFormData({
      ...
      setSending(false);
      const msg = err.response?.data?.error?.message || 'Failed to send request. Please try again or contact us directly.';
      setError(msg);
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="relative px-8 pt-8 pb-4">
          ...
          <h2 className="font-display text-2xl font-bold text-navy-900 tracking-tight">Request a Quote</h2>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-navy-800 mb-1.5 uppercase tracking-wider">Product Interest *</label>
